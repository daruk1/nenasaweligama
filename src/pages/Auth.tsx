        ...
        <div className="mx-auto max-w-4xl mb-12">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <FadeIn delay={150} direction="left">
              <div className="group overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--card-shadow)] transition-all duration-300 hover:shadow-[var(--card-hover-shadow)] hover:scale-[1.02]">
                <div className="overflow-hidden">
                  <img src={englishPromo} alt="4 Month English Programme" className="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" />
                </div>
                <div className="p-5">
                  <h3 className="font-display text-lg font-bold text-foreground">4 Month English Programme</h3>
            ...
            <FadeIn delay={300} direction="right">
              <div className="group overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--card-shadow)] transition-all duration-300 hover:shadow-[var(--card-hover-shadow)] hover:scale-[1.02]">
                <div className="overflow-hidden">
                  <img src={englishPromo2028} alt="A/L General English 2028" className="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" />
                </div>
                <div className="p-5">
                  <h3 className="font-display text-lg font-bold text-foreground">A/L General English 2028</h3>
        ...
        <FadeIn delay={500}>
          <Card className="mx-auto max-w-md border-0 shadow-[var(--card-shadow)]">
            <CardHeader className="text-center">
              ...
              <CardDescription>{isLogin ? "Sign in to register for classes" : "Sign up to get started with Nenasa Education"}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <Button variant="outline" className="w-full gap-2 h-11" onClick={() => handleOAuth("google")}>
                ...
                <Button variant="outline" className="w-full gap-2 h-11" onClick={() => handleOAuth("apple")}>
              ...
              <form onSubmit={handleEmailAuth} className="space-y-4">
                {!isLogin && (
                  <div className="space-y-2">
                    <Label htmlFor="fullName" className="flex items-center gap-1"><User className="h-3.5 w-3.5"/> Full Name</Label>
                ...
                <Button type="submit" className="w-full bg-accent text-accent-foreground hover:bg-accent/90 font-semibold gap-2" size="lg" disabled={loading}>
                  <LogIn className="h-4 w-4"/>{loading ? "Please wait..." : isLogin ? "Sign In" : "Sign Up"}
                </Button>
              </form>
              <p className="text-center text-sm text-muted-foreground">
                {isLogin ? "Don't have an account?" : "Already have an account?"}{" "}
                <button onClick={() => setIsLogin(!isLogin)} className="text-accent font-semibold hover:underline">{isLogin ? "Sign Up" : "Sign In"}</button>
              </p>
            </CardContent>
          </Card>
        </FadeIn>
